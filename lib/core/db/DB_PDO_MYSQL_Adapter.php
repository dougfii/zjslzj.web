<?php
class DB_PDO_MYSQL_Adapter extends DB_Adapter
{
	...
	public function __construct($conf, $driver = 'mysql')
	{
		if (isset ( $conf ['unix_socket'] ) && $conf ['unix_socket'] [0] == '/')
		{
			$this->dsn = $driver . ":dbname={$conf['dbname']};unix_socket={$conf['unix_socket']}";
		}
		else if ($conf ['port'] [0] == '/')
		{
			$this->dsn = $driver . ":dbname={$conf['dbname']};unix_socket={$conf['port']}";
		}
		else
		{
			$this->dsn = $driver . ":dbname={$conf['dbname']};host={$conf['host']};port={$conf['port']}";
		}
	...
	{
		if (! ($size === 0 && $start === 0)) return "LIMIT {$start}, {$size}";
		return '';
	}
}
?>