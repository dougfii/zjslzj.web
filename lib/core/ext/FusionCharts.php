...
class FusionCharts
{
	...
	function renderChart($renderAsHTML = false, $display = false)
	{
	...
	function encodeSpecialChars($strValue)
	{
		...
		if ($this->encodeChars == true)
		{
            ...
            $strValue = preg_replace_callback($pattern, function () {
                return "%25";
            }, $strValue);
            ...
            $strValue = preg_replace_callback("/\<br/i", function () {
                return "%26lt;BR";
            }, $strValue);
	...
	function renderChartMap($swf, $url, $xml, $id, $width, $height, $debug = false, $js = false, $trans = "")
	{
		$s = "";
		
		if ($xml == "") $tmp = "chart_$id.setDataURL(\"$url\");";
		else $tmp = "chart_$id.setDataXML(\"$xml\");";
		
		$div = $id . "Div";
		$debug = $this->bool ( $debug );
		$js = $this->bool ( $js );
		$nsetTransparent = ($trans ? "true" : "false");
		
		$s .= "<div id=\"$div\"></div>\n";
		$s .= "<script type=\"text/javascript\" src=\"fm.js\"></script>\n";
		$s .= "<script type=\"text/javascript\" >\n";
		$s .= "var chart_$id = new FusionMaps(\"$swf\", \"$id\", \"$width\", \"$height\", \"$debug\", \"$js\", \"" . $this->JSC ["bgcolor"] . "\",\"" . $this->JSC ["scalemode"] . "\",\"" . $this->JSC ["lang"] . "\"); \n";
		
		if ($trans == "true")
		{
			$s .= "chart_$id.setTransparent(\"$trans\");\n";
		}
		
		$s .= $tmp . "\n";
		$s .= "chart_$id.render(\"$div\");\n";
		$s .= "</script>\n";
		
		return $s;
	}
	
	function renderChartJS($swf, $url, $xml, $id, $width, $height, $debug = false, $js = false, $trans = "")
	{
		$s = "";
		...
		$s .= "<div id=\"$div\"></div>\n";
		$s .= "<script type=\"text/javascript\" >\n";
		$s .= "var chart_$id = new FusionCharts(\"$swf\", \"$id\", \"$width\", \"$height\", \"$debug\", \"$js\", \"" . $this->JSC ["bgcolor"] . "\",\"" . $this->JSC ["scalemode"] . "\",\"" . $this->JSC ["lang"] . "\"); \n";
	...
	function renderChartHTML($swf, $url, $xml, $id, $width, $height, $debug = false, $js = false, $trans = "")
	{
		$s = "";
		$v = "&chartWidth=" . $width . "&chartHeight=" . $height . "&debugMode=" . $this->bool ( $debug ) . "&scaleMode=" . $this->JSC ["scalemode"] . "&lang=" . $this->JSC ["lang"];
		
		if ($xml == "") $v .= "&dataURL=" . $url;
		else $v .= "&dataXML=" . $xml;
		
		$js = $this->bool ( $js );
		if ($trans != "") $trans = $trans == false ? "opaque" : "transparent";
		else $trans = "window";
		...
		if ($pos === false)
		{
			$s .= "<embed src=\"$swf\" FlashVars=\"$v&registerWithJS=$js\" quality=\"high\" width=\"$width\" height=\"$height\" name=\"$id\" ";
			$s .= ($this->JSC ["bgcolor"] != "") ? " bgcolor=\"" . $this->JSC ["bgcolor"] . "\"" : "";
			$s .= " allowScriptAccess=\"always\"  type=\"application/x-shockwave-flash\" pluginspage=\"$HTTP://www.macromedia.com/go/getflashplayer\" wmode=\"$trans\" /> \n";
		}
		else
		{
			...
			$s .= "<param name=\"FlashVars\" value=\"$v&registerWithJS=$js\" /> \n";
			$s .= "<param name=\"quality\" value=\"high\"  /> \n";
			$s .= "<param name=\"wmode\" value=\"$trans\"  /> \n";
			
			if ($this->JSC ["bgcolor"] != "")
			{
				$s .= "<param name=\"bgcolor\" value=\"" . $this->JSC ["bgcolor"] . "\" /> \n";
			}
