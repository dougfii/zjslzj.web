...
class Flow4Cls
{
    ...
    public static function Add($pid, $name, $no, $code, $date, $content, $personals, $progress, $writer, $writer_date, $signer, $signer_date)
    {
        $rs = DB::db()->Fetch("
                INSERT INTO " . self::TABLE . "(pid, name, no, code, date, content, personals, progress, writer, writer_date, signer, signer_date)
				VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
				RETURNING id
				", array($pid, $name, $no, $code, $date, $content, $personals, $progress, $writer, $writer_date, $signer, $signer_date));
        return $rs ['id'];
    }
    ...
    public static function GetReplyItems($pid)
    {
        $rs = self::Items("WHERE del=false AND pid={$pid} AND replyid>0", 'ORDER BY id DESC', 0, 0);
        return $rs;
    }

    public static function GetProjectItems($pid)
    {
        $rs = self::Items("WHERE del=false AND pid={$pid}", 'ORDER BY id DESC', 0, 0);
        return $rs;
    }
}