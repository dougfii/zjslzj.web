...
class Reply9999Cls
{
    ...
    public static function Add($pid, $fid, $no, $signer, $content, $comp, $date, $uid, $act)
    {
        $rs = DB::db()->Fetch("
                INSERT INTO " . self::TABLE . "(pid, fid, no, signer, content, comp, date, uid, act)
				VALUES(?,?,?,?,?,?,?,?,?)
				RETURNING id
				", array($pid, $fid, $no, $signer, $content, $comp, $date, $uid, $act));
        return $rs ['id'];
    }
