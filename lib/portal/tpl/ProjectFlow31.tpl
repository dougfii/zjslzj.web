<div class="toolbar">
    <div class="tool clear"><span class="cap">外观质量检查与评定项目确认 - [<?php echo $state; ?>]</span><a class="tooladd back" href="javascript:;">返回</a><?php if($edit) echo '<a class="tooladd" id="add" href="javascript:;">提交</a>'; ?></div>
</div>
...
<script>
    $(function(){
        $('#add').click(function(){
            ...
            $.post('?m=Project&a=OnProjectFlow31', {id:$('#id').val(),name:$('#name').val(),no:$('#no').val(),signer:$('#signer').val(),content:$('#content').val(),date:$('#date').val(),comp:$('#comp').val(),date_ping:$('#date_ping').val()
        ...
        $('#edititems').click(function(){layer.pop('?m=Project&a=PopFacadeItemsEdit&id=3','编辑项目');});
