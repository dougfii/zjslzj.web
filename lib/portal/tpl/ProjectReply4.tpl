<script>
    $(function(){
        $('#add').click(function(){
            $.post('?m=Project&a=OnProjectReply4', {fid:$('#fid').val(),no:$('#no').val(),signer:$('#signer').val(),content:$('#content').val(),date:$('#date').val(),comp:$('#comp').val()}, function (ret){if(ret.code==1)layer.msg('提交成功', 1, function(){history.back();});else layer.msg(ret.msg, 1);}, 'json');
        });
