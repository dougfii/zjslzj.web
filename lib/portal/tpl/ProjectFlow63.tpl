<script>
    $(function(){
        $('#add').click(function(){
            ...
            $.post('?m=Project&a=OnProjectFlow63', {id:$('#id').val(),name:$('#name').val(),no:$('#no').val(),signer:$('#signer').val(),date:$('#date').val()
            ,v1c:$('#v1c').val(),v1n:$('#v1n').val(),v1d:$('#v1d').val(),v2c:$('#v2c').val(),v2n:$('#v2n').val(),v2d:$('#v2d').val(),v3c:$('#v3c').val(),v3n:$('#v3n').val(),v3d:$('#v3d').val()
            ,total0:$('#total0').val(),total1:$('#total1').val(),total2:$('#total2').val(),total3:$('#total3').val(),total4:$('#total4').val(),total5:$('#total5').val()
        ...
        $('#items').on('blur', '.item2', function(){
            $.each($('.item2'), function(i, n){
                ...
                if(v != '' && isNaN(v)){
                    ...
                    if (x != '' && !isNaN(x)) {
                        layer.msg('条目序号' + (i + 1) + '的优良数和合格数只能填写一项', 2, -1);
                        return false;
                    }
                }
            });
        ...
        function totals(){
            ...
            var v = v2+v3;
            if(v==0) $('#total2').val('0');
            else $('#total2').val(((v2/v)*100).toFixed(2));
        }
    });
</script>
<div class="toolbar">
    <div class="tool clear"><span class="cap">单位工程质量等级核定表 - [<?php echo $state; ?>]</span><a class="tooladd back" href="javascript:;">返回</a><?php if($edit) echo '<a class="tooladd" id="add" href="javascript:;">提交</a>'; ?></div>
</div>
<div class="panel paneltool">
    <div class="pagea4">
        <div class="pagea4info">
            <div class="center">单位工程质量等级核定表</div>
            <table class="tx1">
                <tr><td class="k">单位工程名称</td><td class="v"><?php if($edit) echo '<input type="text" class="" id="name" value="' . $name . '" />'; else echo $name; ?></td><td class="k">项目法人</td><td class="v"><?php if($edit) echo '<input type="text" class="" id="signer" value="' . $signer . '" />'; else echo $signer; ?></td></tr>
                <tr><td class="k">单位工程编号</td><td class="v"><?php if($edit) echo '<input type="text" class="" id="no" value="' . $no . '" />'; else echo $no; ?></td><td class="k">核定时间</td><td class="v"><?php if($edit) echo '<input type="text" class="" id="date" value="' . $date . '" onclick="laydate();" readonly />'; else echo $date; ?></td></tr>
            </table>
            <table class="tx1">
                <thead>
                    <tr><td rowspan="2">序号</td><td rowspan="2">分部工程名称与编号</td><td colspan="2">核定等级</td></tr>
                ...
                <tfoot>
                    <tr><td colspan="4">分部工程共 <?php if($edit) echo '<input type="text" class="c" id="total0" size="5" value="' . $total0 . '" />'; else echo $total0; ?> 个，其中优良 <?php if($edit) echo '<input type="text" class="c" id="total1" size="5" value="' . $total1 . '" />'; else echo $total1; ?> 个，优良率 <?php if($edit) echo '<input type="text" class="c" id="total2" size="5" value="' . $total2 . '" />'; else echo $total2; ?> %，主要分部工程优良率 <?php if($edit) echo '<input type="text" class="c" id="total3" size="5" value="' . $total3 . '" />'; else echo $total3; ?> %，外观工程质量得分 <?php if($edit) echo '<input type="text" class="c" id="total4" size="5" value="' . $total4 . '" />'; else echo $total4; ?>，单位工程质量等级 <?php if($edit) echo '<input type="text" class="c" id="total5" size="5" value="' . $total5 . '" />'; else echo $total5; ?> 。</td></tr>
                </tfoot>
            </table>
