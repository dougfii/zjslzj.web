...
if(!empty($rs_new) || !empty($rs_processing) || !empty($rs_backed) || !empty($rs_success))
{
    if(!empty($rs_new))
    {
        echo '<div>新建</div>';
         ...
         {
            echo '<tr><td><a href="?m=Work&a=Item&work_id=' . $v['work_id'] . '&node_id=' . $v['node_id'] . '&item_id=' . $v['id'] . '">' . $v['no'] . ' - ' . $v['time'] . '</a></td><td class="ftim">' . $v['time'] . '</td><td class="fsta">' . $v['status'] . '</td></tr>';
         }
         echo '</table><br/>';
    }
    ...
    if(!empty($rs_backed))
    {
         ...
         {
             echo '<tr><td>' . $rr['content'] . '</td><td class="fusr">' . $rr['user'] . '</td><td class="ftim">' . $rr['time'] . '</td></tr>';
         }
         echo '</table><br/>';
    }
