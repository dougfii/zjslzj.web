<script>
    $(function(){
        $('#allow').click(function(){
            $.post('?m=Project&a=OnProjectFlow63Allow', {pid:$('#pid').val(),fid:$('#fid').val(),content:$('#content').val()}, function (ret){if(ret.code==1)layer.msg('批复完成', 1, function(){location.reload();});else layer.msg(ret.msg, 1);}, 'json');
        });

        $('#deny').click(function(){
            $.post('?m=Project&a=OnProjectFlow63Deny', {pid:$('#pid').val(),fid:$('#fid').val(),content:$('#content').val()}, function (ret){if(ret.code==1)layer.msg('批复完成', 1, function(){location.reload();});else layer.msg(ret.msg, 1);}, 'json');
        });
    });
</script>
<div class="toolbar">
    <div class="tool clear"><span class="cap">单位工程质量等级核定表 - [<?php echo $state; ?>]</span><a class="tooladd back" href="javascript:;">返回</a></div>
</div>
<div class="panel paneltool">
    ...
    <div class="pagea4">
        <div class="pagea4info">
            ...
            <table class="tx1">
                <tr><td class="k">单位工程名称</td><td class="v"><?php echo $rs['name']; ?></td><td class="k">项目法人</td><td class="v"><?php echo $rs['signer']; ?></td></tr>
            ...
            <table class="tx1">
                ...
                <tfoot>
                    <tr><td colspan="4">分部工程共 <?php echo $total0; ?> 个，其中优良 <?php echo $total1; ?> 个，优良率 <?php echo $total2; ?> %，主要分部工程优良率 <?php echo $total3; ?> %，外观工程质量得分 <?php echo $total4; ?>，单位工程质量等级 <?php echo $total5; ?> 。</td></tr>
                </tfoot>
            </table>
            <table class="tx1">
