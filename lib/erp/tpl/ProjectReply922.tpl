<script>
    $(function(){
        $('#allow').click(function(){
            $.post('?m=Project&a=OnProjectReply922', {fid:$('#fid').val(),proj:$('#proj').val(),stage:$('#stage').val(),signer:$('#signer').val(),signer_code:$('#signer_code').val(),owner:$('#owner').val(),owner_code:$('#owner_code').val(),comp:$('#comp').val(),date:$('#date').val()
...
<div class="toolbar">
    <div class="tool clear"><span class="cap">政府验收 - 部分工程投入验收 - [批复]</span><a class="tooladd back" href="javascript:;">返回</a></div>
</div>
