<script>
	...
	$(function(){
		...
		$('#ok').click(function(){
			$.post('?m=System&a=OnFacadeTypeAdd', {pid:$('#pid').val(),name:$('#name').val(),score:$('#score').val()}, function (ret){if(ret.code==1)layer.msg('增加成功', 1, function(){window.history.back();});else layer.msg(ret.msg, 1);}, 'json');
		});
	});
</script>
