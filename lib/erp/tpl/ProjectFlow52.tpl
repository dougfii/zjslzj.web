<script>
    $(function(){
        $('#allow').click(function(){
            $.post('?m=Project&a=OnProjectFlow52Allow', {pid:$('#pid').val(),fid:$('#fid').val(),content:$('#content').val()}, function (ret){if(ret.code==1)layer.msg('批复完成', 1, function(){location.reload();});else layer.msg(ret.msg, 1);}, 'json');
        });

        $('#deny').click(function(){
            $.post('?m=Project&a=OnProjectFlow52Deny', {pid:$('#pid').val(),fid:$('#fid').val(),content:$('#content').val()}, function (ret){if(ret.code==1)layer.msg('批复完成', 1, function(){location.reload();});else layer.msg(ret.msg, 1);}, 'json');
        });
    });
</script>
<div class="toolbar">
    <div class="tool clear"><span class="cap">阶段验收分部工程质量核备 - [<?php echo $state; ?>]</span><a class="tooladd back" href="javascript:;">返回</a></div>
</div>
    ...
    <div class="pagea4">
        <div class="pagea4info">
            ...
            <table class="tx1">
                ...
                <tr><td class="k">单位工程编号</td><td class="v"><?php echo $rs['no']; ?></td><td class="k">核备时间</td><td class="v"><?php echo $rs['date']; ?></td></tr>
            </table>
            <table class="tx1">
                <thead>
