...
<div class="panel paneltool">
    ...
    if(!empty($rl)){
        ...
        echo '</tr>';
        echo '<tr>';
        echo '<td><a href="?m=Project&a=ProjectFlow10007&pid=' . $pid . '">' . $rl['name'] . ' ' . $rl['time'] . '</a></td>';
    ...
    if(!empty($rs)){
        ...
        foreach($rs as $k => $v){
            echo '<tr>';
            echo '<td><a href="?m=Project&a=ProjectFlow10007&pid=' . $pid . '&id=' . $v['id'] . '">' . $v['name'] . ' ' . $v['time'] . '</a></td>';
            echo '<td class="ftim">' . $v['time'] . '</td>';
            echo '<td class="fusr">' . $v['user'] . '</td>';
            echo '<td class="ftim">' . $v['replytime'] . '</td>';
            echo '<td class="fsta"><a href="?m=Project&a=ProjectReply10007View&pid=' . $pid . '&fid=' . $v['id'] . '">查看</a></td>';
            echo '</tr>';
        }
        echo '</table>';
    }
